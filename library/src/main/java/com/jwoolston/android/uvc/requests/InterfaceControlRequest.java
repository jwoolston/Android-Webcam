 ...
 */
public abstract class InterfaceControlRequest extends VideoClassRequest {

    private static short valueFromControlSelector(@NonNull ControlSelector selector) {
        return ((short) (0xFFFF & (selector.code << 8)));
    }

    protected InterfaceControlRequest(@NonNull Request request, ControlSelector selector, short index,
                                      @NonNull byte[] data) {
        super(request == Request.SET_CUR ? VC_SET_REQUEST_TYPE : VC_GET_REQUEST_TYPE,
